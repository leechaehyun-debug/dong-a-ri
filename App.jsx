  ...
  const loadMonth = useCallback(async (targetDate) => {
    ...
    try {
      if (!supabaseReady) {
        ...
        setRoster(fresh);
        setToast("Supabase 환경변수를 먼저 설정해주세요.");
        return;
      }
      ...
      // 이번 달 데이터가 없으면 최근 6개월 안의 가장 최신 데이터를 이어받습니다.
      const priorDates = prevMonths(targetDate, 6);
      const priorKeys = priorDates.map(monthKey);
      let base = null;

      if (priorKeys.length) {
        const { data: priorRows, error: priorError } = await supabase
          .from("monthly_rosters")
          .select("month_key, roster")
          .in("month_key", priorKeys);

        if (priorError) throw priorError;

        const byMonth = new Map((priorRows || []).map((row) => [row.month_key, row.roster]));
        for (const key of priorKeys) {
          if (byMonth.has(key)) {
            base = byMonth.get(key);
            break;
          }
        }
      }
      ...
      setRoster(fresh);
      setToast("데이터를 불러오지 못했어요. Supabase 설정을 확인해주세요.");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const saveRoster = useCallback(
    async (next) => {
      ...
      try {
        ...
        console.error(e);
        setRoster(previous);
        setToast("저장에 실패했어요. Supabase 설정을 확인해주세요.");
      } finally {
        setSaving(false);
      }
    },
